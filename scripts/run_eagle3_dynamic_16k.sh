...
python3 -u /data/speculators-dev/scripts/train.py \
    ...
    --save-path /data/training/eagle3_dynamic_16k_nv \
    --noise-std 0.05 --norm-before-residual \
    --pretrain-weights /data/models/Kimi-K25-eagle3/model.safetensors \
    --no-resume-from-checkpoint \
    --ttt-steps 1 --scheduler-type cosine --scheduler-warmup-steps 200 \
    --epoch-samples 5000 --max-checkpoints 3 --enforce-eager --loss-type ce \
    "$@"
